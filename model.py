...
class EnrichedTiSASRec(torch.nn.Module): # similar to torch.nn.MultiheadAttention
    def __init__(self, user_num, item_num, time_num, cat_num, args):
        ...
        self.cat_K_emb = torch.nn.Embedding(100, args.hidden_units) # new category embedding Key, hardcode 100 for percentage of each category
        self.cat_V_emb = torch.nn.Embedding(100, args.hidden_units) # new category embedding Value, hardcode 100 for percentage of each category
    ...
    def seq2feats(self, user_ids, log_seqs, rating_seqs, cat_seqs, time_matrices):
        ...
        ratings = torch.LongTensor(rating_seqs).to(self.dev) # gaussan fn
        ...
        categories = torch.LongTensor(cat_seqs).to(self.dev) # try cross-attention
