...
def Relation(user_train, usernum, maxlen, time_span):
    ...
    for user in tqdm(range(1, usernum+1), desc='Preparing relation matrix'):
        ...
        for i in reversed(user_train[user][:-1]):
            ...
            idx -= 1
            if idx == -1: break
        relation_matrix[user] = computeRePos(time_seq, time_span)
...
def sample_function(user_train, usernum, itemnum, batch_size, maxlen, relation_matrix, result_queue, SEED):
    def sample(user):
        ...
        rating_seq = np.zeros([maxlen], dtype=np.int32)
        ...
        return (user, seq, rating_seq, time_seq, time_matrix, cat_seq, pos, neg)
...
def cleanAndsort(User, time_map):
    ...
    User_res = dict()
    for user, items in User_filted.items():
        User_res[user] = list(map(lambda x: [x[0], time_map[x[1]], x[2], x[3]], items))
    ...
    return User_res, len(user_set), len(item_set), max(time_max), len(cat_set),
...
def data_partition(fname):
    ...
    # the first loop counts users and items
    for line in f:
        try:
            u, i, r, timestamp, c = line.rstrip().split('\t')
        except:
    ...
    for line in f:
        ...
        User[u].append([i, timestamp, r, c])
    f.close()
    
    time_map = timeSlice(time_set)
    User, usernum, itemnum, timenum, cat_num = cleanAndsort(User, time_map)
    ...
    return [user_train, user_valid, user_test, usernum, itemnum, timenum, cat_num]


def evaluate(model, dataset, args):
    [train, valid, test, usernum, itemnum, timenum, catnum] = copy.deepcopy(dataset)
    ...
    for u in users:
        ...
        rat_seq = np.zeros([args.maxlen], dtype=np.int32)
        cat_seq = np.zeros([args.maxlen], dtype=np.int32)
        time_seq = np.zeros([args.maxlen], dtype=np.int32)
        idx = args.maxlen - 1
        
        seq[idx] = valid[u][0][0]
        rat_seq[idx] = valid[u][0][2]
        cat_seq[idx] = valid[u][0][3]
        ...
        predictions = -model.predict(*[np.array(l) for l in [[u], [seq], [rat_seq], [cat_seq], [time_matrix], item_idx]])
    ...
    return NDCG / valid_user, HT / valid_user, NDCG_20 / valid_user, HR_20 / valid_user
